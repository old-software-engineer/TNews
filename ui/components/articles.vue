    ...
    <div class="bg-white px-4 pt-16 pb-20 sm:px-6 lg:px-8 lg:pt-15 lg:pb-28">
      <div class="relative mx-auto max-w-lg lg:max-w-7xl">
            ...
            <Menu as="div" class="relative inline-block text-left">
              ...
              <transition enter-active-class="transition ease-out duration-100"
                ...
                <MenuItems
                  ...
                  <div class="py-1">
                    <MenuItem v-for="option in types" :key="option">
                    <a class="text-gray-500 block px-4 py-2 text-sm hover:cursor-pointer">{{
                        option
                    }}</a>
                    </MenuItem>
                  </div>
              ...
              <transition enter-active-class="transition ease-out duration-100"
                ...
                <MenuItems
                  ...
                  <div class="py-1">
                    ...
                    <NuxtLink class="text-gray-500 block px-4 py-2 text-sm hover:cursor-pointer"
                      @click="handleClick(category.name, category.id)">{{ category.name }}</NuxtLink>
                    </MenuItem>
                  </div>
                </MenuItems>
              </transition>
        ...
        <div class="grid gap-16 pt-12 lg:grid-cols-3 lg:gap-x-5 lg:gap-y-12">
          <ArticleCard v-for="post in posts" :key="post.title" :post="post" />
        </div>
      </div>
    </div>
  </div>
